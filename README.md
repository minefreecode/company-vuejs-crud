...
5.   Отдельное внимание можно уделить работе с компонентами. Потому что в проекте данный процесс полностью автоматизирован
так, что для добавления целого справочника необходимо только добавление компонента `<CRUD>` на настройками к какому `API`
будет обращаться данный компонент для реализации возможностей `CRUD`. Также в файле `mixins/fields.js` указываются поля
сущности, которые будут отображаться и редактироваться. Очень удобно реализовано. И добавление нового справочника `CRUD`
происходит максимально быстро.

6. Описание основного компонента `CRUD`:
